...
#---------------------------------------------------------------------------------------------------
#	Provided Package:  libeipa
#---------------------------------------------------------------------------------------------------

PACKAGES-$(PTXCONF_LIBEIPA) += libeipa
...
#---  libeipa package paths  ----------------------------------------------------
DEV_NAME 			:=	eipa
LIB_VERSION_MAJOR	:=	1
LIB_VERSION_MINOR	:=	0
LIB_VERSION			:=	$(LIB_VERSION_MAJOR).$(LIB_VERSION_MINOR)
BT_LIB_NAME			:=	lib$(DEV_NAME)-$(LIB_VERSION)
BT_LIB_DIR			:=	$(BUILDDIR)/$(BT_LIB_NAME)
# LIBMBS_LIBDIR			:= $(LIBMBS_DIR)/libs
# LIBMBS_CONFDIR			:= $(LIBMBS_DIR)/config
LOCAL_LIB_SOURCE	:= $(PTXDIST_WORKSPACE)/wago_intern/device/eip/libeipa/source
# 
...
$(STATEDIR)/libeipa.get:
		@$(call targetinfo)
		@echo "PTX builddir:    $(BUILDDIR)"
		@echo "PTX srcdir:      $(SRCDIR)"
		@echo "PTX sysroot target:  $(PTXCONF_SYSROOT_TARGET)"
		@echo "libeipa source:  $(LOCAL_LIB_SOURCE)"
		@echo "libeipa dir:     $(BT_LIB_DIR)"
		@echo
		@$(call touch)


#---------------------------------------------------------------------------------------------------
#   Extract			Create libeipa directory in the <build-target> directory and synchronize it
#					with local source directories
#---------------------------------------------------------------------------------------------------

$(STATEDIR)/libeipa.extract:
		@$(call targetinfo)
		@echo "Create build-target directory:  $(BT_LIB_DIR)"
		$(NOPRINT)mkdir -p $(BT_LIB_DIR)
		@echo "Synchronize local project directories into build-target ..."
		rsync  $(RSYNC_OPTIONS)  $(LOCAL_LIB_SOURCE)  $(BT_LIB_DIR)
		rsync  $(RSYNC_OPTIONS)  $(LOCAL_LIB_SOURCE)/../build  $(BT_LIB_DIR)
		@echo
		@$(call touch)
...
$(STATEDIR)/libeipa.prepare:
		@$(call targetinfo)
		@echo "Create additional directories in the build-target ..."
		$(NOPRINT)mkdir -p $(BT_LIB_DIR)/lib
...
#		@echo "Create diagnostic header file for libeipa in sysroot-target/usr/include/diagnsostic directory by using of XSLT-script"
...
LIBEIPA_ENV 	:= $(CROSS_ENV)

$(STATEDIR)/libeipa.compile:
		@$(call targetinfo)
		@echo "Build library libeipa ..."
		cd $(BT_LIB_DIR)/source  &&  $(LIBEIPA_ENV) \
						DIST_DIR=$(PTXDIST_PLATFORMDIR) \
...
#					sysroot-target. Creat a symbolic link onto libeipa versioned shared lib file.
#					The logging mask config file must be copied into the /etc/specific directory
#---------------------------------------------------------------------------------------------------

$(STATEDIR)/libeipa.install:
		@$(call targetinfo)
...
#---------------------------------------------------------------------------------------------------
#   Target-Install	Install package on target. Copy the libeipa lib into <root> and <root-debug>
#                   directories of the target platform and create corresponding links.
#---------------------------------------------------------------------------------------------------

$(STATEDIR)/libeipa.targetinstall:
		@$(call targetinfo)
#		@$(call install_init,   libeipa)
#		@$(call install_fixup,  libeipa, VERSION, $(LIBMBS_VERSION))
#		@$(call install_fixup,  libeipa, PRIORITY, optional)
#		@$(call install_fixup,  libeipa, SECTION, base)
#		@$(call install_fixup,  libeipa, AUTHOR, "Wago")
#		@$(call install_fixup,  libeipa, DESCRIPTION, missing)
#ifdef PTXCONF_WAGO_TOOLS_BUILD_VERSION_BINARIES 
...
#		@$(call install_archive, libeipa, 0, 0, $(LIBMBS_PLATFORMCONFIGPACKAGEDIR)/tmp/data.tar.gz, /)
#else
#        ##  WAGO_TOOLS_BUILD_VERSION_TRUNK | WAGO_TOOLS_BUILD_VERSION_RELEASE
#		@$(call install_copy,   libeipa, 0, 0, 0755, $(LIBMBS_LIBDIR)/$(LIBMBS).so.$(LIBMBS_VERSION), /usr/lib/$(LIBMBS).so.$(LIBMBS_VERSION))
#		@$(call install_link,   libeipa, $(LIBMBS).so.$(LIBMBS_VERSION), /usr/lib/$(LIBMBS).so)
#		@$(call install_link,   libeipa, $(LIBMBS).so.$(LIBMBS_VERSION), /usr/lib/$(LIBMBS).so.$(LIBMBS_VERSION_MAJOR))
#		@$(call install_link,   libeipa, ../$(LIBMBS).so.$(LIBMBS_VERSION), /usr/lib/dal/$(LIBMBS).so)
#		@$(call install_copy,   libeipa, 0, 0, 0755, $(LIBMBS_CONFDIR)/mbslog.conf, /etc/specific/mbslog.conf)	
#endif
#		@$(call install_finish, libeipa)
#ifdef PTXCONF_WAGO_TOOLS_BUILD_VERSION_RELEASE
...
#---------------------------------------------------------------------------------------------------
# 	Clean			Uninstall package libeipa and remove all created and copied shared objects and
#					link to them.
#---------------------------------------------------------------------------------------------------

$(STATEDIR)/libeipa.clean:
		@echo
		@echo --------------------
		@echo target: clean libeipa
		@echo --------------------
		@echo
		@echo "statedir:  $(STATEDIR)"
		@echo "pkgdir:    $(PKGDIR)"
		@echo
		rm -rf $(STATEDIR)/libeipa*
		rm -rf $(PKGDIR)/libeipa*
		rm -rf $(BT_LIB_DIR)
...
#		rm -f  $(PTXCONF_SYSROOT_TARGET)/usr/include/mbs_*.h
		...
		@echo
		@echo finished target clean libeipa
		@echo
