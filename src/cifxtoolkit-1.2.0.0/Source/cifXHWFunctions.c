...
***************************************************************************************
  ...
  $Id: cifXHWFunctions.c 6602 2014-10-02 14:36:27Z stephans $:
  ...
  Changes:
    ...
    2013-02-11  Removed unneccessary readout of Sync handshake cells in DEV_IsReady/DEV_IsRunning/
                DEV_GetHandshakeBitState (improves performance on slow busses)
    2011-09-13  Added DEV_DoSystemBootstart()
    2011-03-29  Lock DEV_DoSystemStart againt DSR to prevent writes to
    ...
    2009-02-13  DEV_CheckCOSFlags is now locked agains DEV_DoChannelInit,
    ...
    2007-04-26  - Unneccessary DEV_ReadHandshake() removed
...
{
  ptChannel->usHostFlags ^= ulBitMask;
...
{ 
  ...
  ptDevInstance->tSyncData.usHSyncFlags ^= ulBitMask;
...
int DEV_IsCommunicating(PCHANNELINSTANCE ptChannel, int32_t* plError)
{
  ...
  } else
  {
    /* only a Communication channel can be running */
    if( (!ptChannel->fIsSysDevice) &&
        (ptChannel->usNetxFlags & NCF_COMMUNICATING) )
    {
      iRet = 1;
