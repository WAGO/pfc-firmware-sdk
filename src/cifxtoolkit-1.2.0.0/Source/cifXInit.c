...
***************************************************************************************
  ...
  $Id: cifXInit.c 6621 2014-10-14 10:32:33Z Robert $:
                                ...
                                uint32_t        ulTimeout)
{
                           ...
                           ulTimeout))
  {
    ...
    volatile uint8_t*   pbMailbox     = ptHBootData->tToHostMailbox.pvData;
  ...
  for(ulBlockID = ulDPMChannelStartIdx; ulBlockID < NETX_MAX_SUPPORTED_CHANNELS; ++ulBlockID)
  {
    ...
    if(fCreateChannel)
    {
      ...
      if(fCreateChannel)
      {
        ...
        ptDevInstance->pptCommChannels = (PCHANNELINSTANCE*)OS_Memrealloc(ptDevInstance->pptCommChannels, ptDevInstance->ulCommChannelCount * sizeof(*ptDevInstance->pptCommChannels));
        ...
        if( DEV_WaitForReady_Poll(ptChannelInst, 20))
        {
          ...
          lTempError = cifXReadFirmwareIdent( ptDevInstance,
                                              ptChannelInst->ulChannelNumber,
                                              NULL,
                                              NULL);
          UNREFERENCED_PARAMETER(lTempError);
...
static int32_t cifXStopDevice(PDEVICEINSTANCE ptDevInstance)
{
  ...
  } else
  {
    ...
    g_pptDevices = (PDEVICEINSTANCE*)OS_Memrealloc(g_pptDevices, g_ulDeviceCount * sizeof(*g_pptDevices));
  }
