...
***************************************************************************************
  ...
  $Id: cifXFunctions.c 6602 2014-10-02 14:36:27Z stephans $:
  ...
  Changes:
    ...
    2014-08-19  xChannelRegisterNotification/CIFX_NOTIFY_COM_STATE now calls
                ...
                xChannelCommonStatusBlock(), convertion structure s_atCommonStatusBlock
    ...
    2010-04-21  Fixed GCC compiler warning (deferencing pointer breaks strict-aliasing rules)
    ...
    2007-04-17  - xSysdeviceDownload(),
                ...
                - PFN_DOWNLOAD_PROGRESS reneamed into DEV_PROGRESS_CALLBACK
    ...
    case CIFX_INFO_CMD_SYSTEM_CHANNEL_BLOCK:
      ...
      } else
      {
            ...
            ++iChannel)
        {
          ...
          /* Convert endianess */
          switch(ptSysChannel->atChannelInfo[iChannel].tSystem.bChannelType)
          {
          case RCX_CHANNEL_TYPE_SYSTEM:
            cifXConvertEndianess(&ptSysChannel->atChannelInfo[iChannel],
                                 ulBlockLength,
    ...
    case CIFX_GET_EXTENDED_MEMORY_POINTER:
      {
        ...
        }else
        {
          ...
          } else
          {
            ...
            ptExtMemInfo->ulMemoryType = ptSysChannel->tSystemState.ulHWFeatures & (RCX_SYSTEM_EXTMEM_ACCESS_MSK | RCX_SYSTEM_EXTMEM_TYPE_MSK); 
  ...
  if( ptChannel->ulDeviceCOSFlags & RCX_COMM_COS_DMA)
  {
    ...
    } else
    {
      ...
      } else
      {
        ...
        /* Read data done */
        DEV_ToggleBit(ptChannel, 1UL << ptIOArea->bHandshakeBit);
      ...
      switch(ulCmd)
      {
        case CIFX_MEM_PTR_OPEN:
        {
          ...
          void*    pvDPM        = pptIOInstances[ptMemory->ulAreaNumber]->pbDPMAreaStart;
                /*    WAT17821  */
          uint32_t ulDPMSize    = 5000;   //pptIOInstances[ptMemory->ulAreaNumber]->ulDPMAreaLength;
    ...
    } else
    {
          ...
          (lRet != CIFX_DEV_NOT_RUNNING) )
      {
        ...
        *pulReadState = 0;
        ...
        if( (RCX_FLAGS_NONE == bIOBitState)  ||
            (DEV_GetHandshakeBitState(ptChannel, 1UL << ptIOInst->bHandshakeBit) == bIOBitState) )
        {
          *pulReadState = 1;
        }
      }
  ...
  } else
  {
    ...
    } else
    {
          ...
          (lRet != CIFX_DEV_NOT_RUNNING) )
      {
        ...
        DEV_ToggleBit(ptChannel, 1UL << ptIOInst->bHandshakeBit);
