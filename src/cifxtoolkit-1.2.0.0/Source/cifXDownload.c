...
***************************************************************************************
  ...
  $Id: cifXDownload.c 6602 2014-10-02 14:36:27Z stephans $:
  ...
  Changes:
    ...
    2007-03-30  - Using now RCX_Public.h instead of MidSys_Public.h
                - Send packets completly initalized
                           ...
                           void*                 pvUser)
{
  ...
  *pfDownload = 1;
  ...
  uSendPkt.tRequest.tData.usFileNameLength   = HOST_TO_LE16( (usFileNameLen + 1) );
                       ...
                       void*                   pvUser)
{
  ...
  uint32_t                      ulBlockSize       = ulMailboxSize -
                                                    sizeof(uRecvPkt.tUploadDataCnf); /* maximum size of each file block */
  int                           fSendAbort        = 0;
  int32_t                       lRetAbort         = CIFX_NO_ERROR;
  int32_t                       lRet              = CIFX_NO_ERROR;
  uint32_t                      ulCurrentId       = 0;
      ...
      while( (ulFileLength > 0) && (CIFX_NO_ERROR == lRet) )
      {
        ...
        } else
        {
          uint32_t  ulCurrentDataLen = LE32_TO_HOST(uRecvPkt.tUploadDataCnf.tHead.ulLen) - 
                                                    sizeof(uRecvPkt.tUploadDataCnf.tData);
