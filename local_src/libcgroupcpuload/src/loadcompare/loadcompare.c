...
//tLoadRing * initLoadRing(uint32_t size);
//tLoadRing * LoadRingRotateAdd(uint32_t load,tLoadRing * pRoot);
//uint32_t LoadRingGetAvg(tLoadRing * pRoot,uint32_t maxCount);
//void procload_read_runtime(tProcLoad *procload);
//void procload_read(tProcLoad *procload);
//uint32_t procload_get_load(tProcLoad *procload,tProcLoadEnum loadEnum);
//void procload_destroy(tProcLoad *procload);
//tProcLoad * _procLoad_init(void);
...
//  tLoadRing * loadRing_PROC = initLoadRing(INTERVALCOUNT(15));
  system("clear");
  printf("/proc/stat (running);cgroups SYS;cgroups RTS\n");
  usleep(100000);
  while(1)
  {
