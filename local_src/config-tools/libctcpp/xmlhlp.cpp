...
/// \file xmlhlp.cpp
/// 
/// \version $Id: xmlhlp.cpp 13475 2015-06-26 14:12:20Z mariusz.podlesny@wago.com $ 
/// 
...
void store_file(const std::string& fname, const xmldoc& doc)
{
    ...
    const int bytes = xmlSaveFormatFileEnc(fname.c_str(), doc.get(), "utf-8", 1);
...
{
    xmlnode node(xmlNewNode(NULL, BAD_CAST name.c_str()));
...
xmlnode append_node(xmlnode& parent, const std::string& name)
{
    ...
    xmlnode child(xmlNewChild(parent.get(), NULL, BAD_CAST name.c_str(), NULL));
...
xmlattr append_attribute(xmlnode& node, const std::string& name, const std::string& value)
{
    ...
    xmlattr attr(xmlNewProp(node.get(), BAD_CAST name.c_str(), BAD_CAST value.c_str()));
...
xmlattr update_attribute(xmlnode& node, const std::string& name, const std::string& value)
{
    ...
    xmlattr attr(xmlSetProp(node.get(), BAD_CAST name.c_str(), BAD_CAST value.c_str()));
...
void remove_attribute(xmlnode& node, const std::string& name)
{
    ...
    xmlattr attr(xmlHasProp(node.get(), BAD_CAST name.c_str()));
...
{
    return xmlpathobj(xmlXPathEvalExpression(BAD_CAST xpath.c_str(), ctx.get()));
}
