...
///
//-----------------------------------------------------------------------------
static int _SprintReallocValue(u_char ** buf, size_t * buf_len,
...
///
//-----------------------------------------------------------------------------
static void _CopySnmpVarData(tWagoSnmpTranceiver * trcv,
                             netsnmp_variable_list * src)
{
                        ...
                        netsnmp_session     * session)
{
  ...
  if (generate_Ku(session->securityAuthProto,
                  ...
                  (u_char *) trcv->sAuthPass, strlen(trcv->sAuthPass),
  ...
  if(session->securityLevel == SNMP_SEC_LEVEL_AUTHPRIV)
  {
    ...
    if (generate_Ku(session->securityAuthProto,
                    session->securityAuthProtoLen,
                    (u_char *) trcv->sPrivPass, strlen(trcv->sPrivPass),
              ...
              const netsnmp_variable_list * variable)
{
    ...
    if (_SprintReallocValue((u_char **) & buf, &buf_len, &out_len, 0,
...
INTERNAL_SYM tWagoSnmpReturnCode INTERNAL_GetSnmpSession(tWagoSnmpTranceiver * trcv, netsnmp_session **ss)
{
  ...
  if(   (trcv->version == SNMP_VERSION_1)
  ...
  if(trcv->trcvType == WAGOSNMP_TRCV_INFORM)
  {
    ...
    if (tmpss.contextEngineIDLen == 0 ||
      ...
      tmpss.contextEngineID =
       snmpv3_generate_engineID(&tmpss.contextEngineIDLen);
    }
#if 1
  ...
  if (!*ss)
  {
    ...
    if(ret == WAGOSNMP_RETURN_OK)
    ret =  WAGOSNMP_RETURN_INIT_SESSION_ERROR;
  }
  return ret;
}
  ...
  *pdu = snmp_pdu_create(type);
  ...
  else if(type == SNMP_MSG_SET || type == SNMP_MSG_INFORM)
  {
    ...
    if(type == SNMP_MSG_INFORM)
    {
      ...
      uptime = get_uptime();
      if (NULL == snmp_pdu_add_variable(*pdu,objid_sysuptime,
                                        sizeof(objid_sysuptime) / sizeof(oid),
                                        ASN_TIMETICKS ,&uptime,sizeof(uptime)))
       {
         snmp_free_pdu(*pdu);
        ...
        &&  SNMP_TLV_INITALIZED(trcv->typData))
    {
      ...
      snmp_pdu_add_variable(*pdu,anOID,anOID_len,SNMP_TLV(trcv->typData)->type,
                            buf,SNMP_TLV(trcv->typData)->val_len);
    }
...
INTERNAL_SYM int INTERNAL_Tranceive(tWagoSnmpTranceiver * trcv, netsnmp_pdu *pdu,netsnmp_session *ss)
{
  ...
  else
  {
    ...
    {
      fprintf(stderr, "Error in packet 0x%X, 0x%X, %s\nReason: %s\n",trcv->trcvType,(unsigned int)trcv->version,trcv->sOID,
...
INTERNAL_SYM int INTERNAL_SendTrapMsg(tWagoSnmpMsg * msg)
{
  ...
  if(mq >= 0)
  {
    ...
    if(0 < poll(&fdsnd, 1, -1))
    {
      ret=mq_send(mq, (const char *)msg,
                          sizeof(tWagoSnmpMsg), 0);
