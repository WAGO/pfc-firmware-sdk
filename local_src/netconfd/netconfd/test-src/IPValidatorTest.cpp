...
namespace netconf
{
  ...
  TEST_F(AnIPValidator, ShouldValidatesIPConfigsThatResetsIPParameterToZero)
  {
  ...
  TEST_F(AnIPValidator, FailedToValidateIPConfigsWithSeveralIpAddresses)
  {
    ...
    IPConfigs ip_configs = {{"ethX1", IPSource::STATIC, "192.168.1.1", "255.255.255.0"},
                            {"ethX2", IPSource::STATIC, "192.168.2.1", "255.255.0.0", }, {"ethX3", IPSource::STATIC,
  ...
  TEST_F(AnIPValidator, validateOverlappingNetworksAsInvalidInCaseOfStaticIP)
  {
    ...
    EXPECT_EQ(StatusCode::INVALID_PARAMETER, status.Get()) << status.GetMessage();
