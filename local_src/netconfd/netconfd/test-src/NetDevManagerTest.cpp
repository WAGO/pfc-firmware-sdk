...
class NetDevManagerTest : public ::testing::Test {
 public:
  ...
  MockIDevicePropertiesProvider mock_dev_prop;
...
TEST_F(NetDevManagerTest, CreatesNetDevsForEachSystemInterface) {
  ...
  EXPECT_CALL(mock_itf_monitor_, RegisterEventHandler(_));
  EXPECT_CALL(mock_bridge_controller_, GetInterfaces()).WillOnce(Return(itfs));
  NetDevManager netdev_manager {shared_itf_monitor_, mock_bridge_controller_ };
...
TEST_F(NetDevManagerTest, ProvidesAllEthernetNetDevs) {

  Interfaces itfs = { "ethX1", "ethX2", "ethX3" };

  EXPECT_CALL(mock_itf_monitor_, RegisterEventHandler(_));
  EXPECT_CALL(mock_bridge_controller_, GetInterfaces()).WillOnce(Return(itfs));
  NetDevManager netdev_manager {shared_itf_monitor_, mock_bridge_controller_ };

  auto netdevs = netdev_manager.GetEthernetNetDevs();

  EXPECT_EQ(itfs.size(), netdevs.size());
...
TEST_F(NetDevManagerTest, DoesNotCopyNetdevObjects) {
  ...
  auto get_ethernet = netdev_manager.GetEthernetNetDevs().front();
