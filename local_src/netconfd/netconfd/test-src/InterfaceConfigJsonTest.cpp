...
TEST_F(InterfaceConfigJsonTest, parseSingleConfig){
  ...
  auto status = converter.InterfaceConfigFromJson(single_port_config, port_configs);
...
TEST_F(InterfaceConfigJsonTest, parseMultipleConfigs){
  ...
  auto status = converter.InterfaceConfigFromJson(multi_port_full, port_configs);
...
TEST_F(InterfaceConfigJsonTest, parseJsonWithWrongContents){
  ...
  auto status = converter.InterfaceConfigFromJson(single_port_config_wrong, port_configs);
...
TEST_F(InterfaceConfigJsonTest, parseJsonWithExtraContents){
  ...
  auto status = converter.InterfaceConfigFromJson(single_port_config_extra_member, port_configs);
...
TEST_F(InterfaceConfigJsonTest, parseJsonMissingContents){
  ...
  auto status = converter.InterfaceConfigFromJson(single_port_config_missing_state, port_configs);
...
TEST_F(InterfaceConfigJsonTest, ConverToJson){
  ...
  auto json = converter.InterfaceConfigToJson(pc);
