...
static Status GetIPParamter(int fd, uint16_t flag, const Interface& interface,
...
static Status SetIPParameter(int fd, uint16_t flag, const Interface& interface,
  ...
  if (1 != inet_pton(AF_INET, value_str.c_str(), &value)) {
    status.Prepend(
        StatusCode::INVALID_PARAMETER,
        "convert error, could not convert ip parameter " + value_str
            + " from presentation to binary format.");
    return status;
  }
...
Status IPController::GetIPConfig(const Interface& interface, IPConfig& config) const {
  ...
  if (-1 == fd) {
    status.Prepend(StatusCode::ERROR,
               "Get ip parameter of interface " + interface + ": open socket error.");
    return status;
  }

  ::std::string ip_value;
  status = GetIPParamter(fd, SIOCGIFADDR, interface, ip_value);
  if (status.NotOk()) {
  ...
  status = GetIPParamter(fd, SIOCGIFNETMASK, interface, netmask_value);
  if (status.NotOk()) {
  ...
  status = GetIPParamter(fd, SIOCGIFBRDADDR, interface, broadcast_value);
  if (status.NotOk()) {
...
Status IPController::SetIPConfig(const IPConfig& config) const {
  ...
  if (-1 == fd) {
    status.Prepend(
        StatusCode::ERROR,
        "Set ip parameter of interface " + config.interface_
            + ": open socket error.");
    return status;
  }
  ...
  if (not config.broadcast_.empty() && "0.0.0.0" != config.broadcast_) {
    status = SetIPParameter(fd, SIOCSIFBRDADDR, config.interface_, config.broadcast_);
    if (status.NotOk()) {
      status.Prepend(
          "Set IP broadcast address of interface " + config.interface_ + ": ");
      return status;
    }
  }
