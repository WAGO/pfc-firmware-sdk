...
void NetworkConfigBrain::Start(StartWithPortstate startwithportstate) {
  ...
  if (status.IsOk()) {
    status = ApplyConfig(bridge_config, ip_configs, dip_switch_ip_config);
  }
...
Status NetworkConfigBrain::ApplyConfig(BridgeConfig &product_config, const IPConfigs &ip_configs, const DipSwitchIpConfig& dipIpConfig) const {
  ...
  if (status.IsOk()) {
    status = ip_manager_.ApplyIpConfiguration(ip_configs, dipIpConfig);
  }
...
::std::string NetworkConfigBrain::Restore(const std::string &file_path) {
  ...
  if (status.IsOk()) {
    ...
    if (status.IsNotOk()) {
      status = ApplyConfig(persisted_bridge_config, persisted_ip_configs, dip_switch_ip_config);
      if (status.IsNotOk()) {
