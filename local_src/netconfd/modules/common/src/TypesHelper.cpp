...
IPConfig& ComplementNetmask(IPConfig &ip_config) {
  if ((IsEmptyOrZeroIP(ip_config.netmask_))
      && (not IsEmptyOrZeroIP(ip_config.address_))) {
    try {
      auto ipv4 = boost::asio::ip::make_address_v4(ip_config.address_);
      auto maskv4 = boost::asio::ip::address_v4::netmask(ipv4);
      ip_config.netmask_ = maskv4.to_string();
    } catch (...) {
      /* nothing to do here */
    }
  }
...
IPConfigs::iterator GetIpConfigByInterface(IPConfigs& configs, const std::string& interfacename){
  return ::std::find_if(configs.begin(), configs.end(), [&](const IPConfig &cfg) {
    return cfg.interface_ == interfacename;
  });
}
...
{
  IPConfigs diff;
  std::set_difference(a.begin(), a.end(), b.begin(), b.end(), std::back_inserter(diff), [](const IPConfig& ia, const IPConfig& ib){return ia.interface_ < ib.interface_;});
  return diff;
}

void SortByInterface(IPConfigs& a)
{
  auto interface_less = [](const IPConfig& ia, const IPConfig& ib){return ia.interface_ < ib.interface_;};
