  ...
  bool operator()(const EthToolLinkSettings& getdata,const EthToolLinkSettings& setdata){
    ...
    return std::tie(
            ...
            (memcmp(&getdata.link_mode_masks_[setdata.s_.link_mode_masks_nwords],
                    ...
                    setdata.s_.link_mode_masks_nwords*3*sizeof(uint32_t))
                != 0);
...
gsl::span<const uint32_t> EthToolSettings::GetSupported() const {
  return std::visit(
      overloaded{[](const EthToolLinkSettings &arg) noexcept {
                 ...
                 [](const ethtool_cmd &arg) noexcept { return gsl::span<const uint32_t>(&arg.supported, 1); }},
      e_);
}
...
gsl::span<const uint32_t> EthToolSettings::GetAutonegAdvertising() const {
  return std::visit(
      overloaded{[](const EthToolLinkSettings &arg) noexcept {
                   return gsl::span<const uint32_t>(&arg.link_mode_masks_[arg.s_.link_mode_masks_nwords],
                                                    arg.s_.link_mode_masks_nwords);
                 },
                 [](const ethtool_cmd &arg) noexcept { return gsl::span<const uint32_t>(&arg.advertising, 1); }},
      e_);
}
...
void EthToolSettings::SetAutonegAdvertising(gsl::span<const uint32_t> autoneg_advertising) {
  ::std::visit(
      overloaded{[&](EthToolLinkSettings &arg) noexcept {
                   ::std::memcpy(
                       &arg.link_mode_masks_[arg.s_.link_mode_masks_nwords], autoneg_advertising.data(),
                       ::std::min(static_cast<size_t>(arg.s_.link_mode_masks_nwords), autoneg_advertising.size()) *
                           sizeof(arg.link_mode_masks_[0]));
                 },
                 [&](ethtool_cmd &arg) noexcept { arg.advertising = *autoneg_advertising.data(); }},
      e_);
}
...
eth::TpMdiType EthToolSettings::GetMdixState() const {
  return std::visit(
      overloaded{
          [](const EthToolLinkSettings &arg) noexcept { return static_cast<eth::TpMdiType>(arg.s_.eth_tp_mdix); },
          [](const ethtool_cmd &arg) noexcept { return static_cast<eth::TpMdiType>(arg.eth_tp_mdix); }},
      e_);
}
...
bool EthToolSettings::IsAutonegEnabled() const {
  return ::std::visit(
      overloaded{[](const EthToolLinkSettings &arg) noexcept { return arg.s_.autoneg == AUTONEG_ENABLE; },
                 [](const ethtool_cmd &arg) noexcept { return arg.autoneg == AUTONEG_ENABLE; }},
      e_);
}
...
void EthToolSettings::SetAutonegEnabled(bool autoneg_enabled) {
  std::visit(
      overloaded{[&](EthToolLinkSettings &arg) noexcept {
                 ...
                 [&](ethtool_cmd &arg) noexcept { arg.autoneg = autoneg_enabled ? AUTONEG_ENABLE : AUTONEG_DISABLE; }},
      e_);
}

eth::Duplex EthToolSettings::GetDuplex() const {
  return ::std::visit(
      overloaded{[](const EthToolLinkSettings &arg) noexcept { return static_cast<eth::Duplex>(arg.s_.duplex); },
                 [](const ethtool_cmd &arg) noexcept { return static_cast<eth::Duplex>(arg.duplex); }},
      e_);
}
...
void EthToolSettings::SetDuplex(eth::Duplex duplex) {
  std::visit(overloaded{[&](EthToolLinkSettings &arg) noexcept {
                        ...
                        [&](ethtool_cmd &arg) noexcept { arg.duplex = static_cast<decltype(arg.duplex)>(duplex); }},
             e_);
}

int EthToolSettings::GetSpeed() const {
  return ::std::visit(overloaded{[](const EthToolLinkSettings &arg) noexcept { return arg.s_.speed; },
                                 [](const ethtool_cmd &arg) noexcept { return ethtool_cmd_speed(&arg); }},
                      e_);
}

void EthToolSettings::SetSpeed(int speed) {
  std::visit(overloaded{[&](EthToolLinkSettings &arg) noexcept { arg.s_.speed = speed; },
                        [&](ethtool_cmd &arg) noexcept { ethtool_cmd_speed_set(&arg, speed); }},
             e_);
}
...
eth::MediaType EthToolSettings::GetMediaType() const {
  return ::std::visit(
      overloaded{[](const EthToolLinkSettings &arg) noexcept { return static_cast<eth::MediaType>(arg.s_.port); },
                 [](const ethtool_cmd &arg) noexcept { return static_cast<eth::MediaType>(arg.port); }},
      e_);
}

EthTool::EthTool(Socket s, const std::string& ifname) : socket_{std::move(s)}, ifreq_{} {
  strncpy(ifreq_.ifr_name, ifname.c_str(), IFNAMSIZ);
...
EthToolSettings EthTool::Create() {
  if (supports_link_settings_) {
    return EthToolSettings{EthToolLinkSettings{}};
  } else {
    return EthToolSettings{ethtool_cmd{}};
  }
}
...
void EthTool::Update(EthToolSettings &s) {
  ...
  IffFlagsGetIoctl();
  s.if_state_ = static_cast<eth::DeviceState>((ifreq_.ifr_flags & IFF_UP) == IFF_UP);
}
...
void EthTool::Commit(EthToolSettings &s) {
  ...
  IffFlagsGetIoctl();
  ifreq_.ifr_flags = static_cast<short>((ifreq_.ifr_flags & ~IFF_UP) | ((s.if_state_ == eth::DeviceState::Up) ? IFF_UP : 0));
  IffFlagsSetIoctl();
